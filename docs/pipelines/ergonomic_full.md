# JSON form: pipeline `ergonomic_full`

Auto-generated from the transformations declared in `gndson.schema`. Do not edit by hand — regenerate with `gndson docs ergonomic_full`.
...
Apply transformations in reverse order: `drop_heterogeneous_inner_tag.inverse` → `collapse_physicalQuantity_wrappers.inverse` → `augment_kind.inverse` → `drop_uniform_inner_tag.inverse` → `enforce_array_arity.inverse`.
...
After applying pipeline `ergonomic_full`:
