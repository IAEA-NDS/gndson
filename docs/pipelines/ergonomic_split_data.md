# JSON form: pipeline `ergonomic_split_data`

Auto-generated from the transformations declared in `gndson.schema`. Do not edit by hand — regenerate with `gndson docs ergonomic_split_data`.
...
7. **`expand_data_columns`** — Heuristic augmentation: parse <data> elements' pipe-separated header comments into _columns and group the text body into _rows (list of lists of token strings).
...
Apply transformations in reverse order: `expand_data_columns.inverse` → `split_whitespace_text.inverse` → `drop_heterogeneous_inner_tag.inverse` → `collapse_physicalQuantity_wrappers.inverse` → `augment_kind.inverse` → `drop_uniform_inner_tag.inverse` → `enforce_array_arity.inverse`.
...
After applying pipeline `ergonomic_split_data`:
