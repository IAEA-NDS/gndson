# JSON form: pipeline `uniform`

Auto-generated from the transformations declared in `gndson.schema`. Do not edit by hand — regenerate with `gndson docs uniform`.
...
Apply transformations in reverse order: `drop_uniform_inner_tag.inverse` → `enforce_array_arity.inverse`.
...
After applying pipeline `uniform`:
